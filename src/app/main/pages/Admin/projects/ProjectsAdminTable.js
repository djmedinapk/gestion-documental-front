...
import ProjectsAdminTablePaginationActions from "./ProjectsAdminTablePaginationActions";
...
const EnhancedTable = ({ columns, data, onRowClick }) => {
  ...
  const paramsDataPageIndex = useSelector(
    ({ projectsAdminApp }) =>
      projectsAdminApp.projects.paramsData.PageIndex
  );
  const paramsDataPageSize = useSelector(
    ({ projectsAdminApp }) =>
      projectsAdminApp.projects.paramsData.PageSize
  );
  const paramsDataCount = useSelector(
    ({ projectsAdminApp }) =>
      projectsAdminApp.projects.paramsData.Count
  );
  ...
  return (
    <div className="flex flex-col w-full min-h-full sm:border-1 sm:rounded-16 overflow-hidden">
      ...
      <TablePagination
        ...
        ActionsComponent={ProjectsAdminTablePaginationActions}
      />
    </div>
  );
};
