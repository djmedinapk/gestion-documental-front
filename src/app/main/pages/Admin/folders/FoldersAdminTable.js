...
import FoldersAdminTablePaginationActions from "./FoldersAdminTablePaginationActions";
import { useDispatch, useSelector } from "react-redux";
import { useTranslation } from "react-i18next";

import { changeParamsDataPageIndex, getFolders } from "./store/foldersAdminSlice";
...
const EnhancedTable = ({ columns, data, onRowClick }) => {
  ...
  const paramsDataPageIndex = useSelector(
    ({ foldersAdminApp }) => foldersAdminApp.folders.paramsData.PageIndex
  );
  const paramsDataPageSize = useSelector(
    ({ foldersAdminApp }) => foldersAdminApp.folders.paramsData.PageSize
  );
  const paramsDataCount = useSelector(
    ({ foldersAdminApp }) => foldersAdminApp.folders.paramsData.Count
  );
  ...
  return (
    <div className="flex flex-col w-full min-h-full sm:border-1 sm:rounded-16 overflow-hidden">
                ...
                {headerGroup.headers.map((column, i) => (
                  ...
                  >
                    {headerGroup.headers.length !== i + 1 ? (
                      t(column.Header.toUpperCase().split(" ").join("_"))
                    ) : (
                      <></>
                    )}
      ...
      <TablePagination
        ...
        ActionsComponent={FoldersAdminTablePaginationActions}
      />
    </div>
  );
};
