...
import VersionsAdminTablePaginationActions from "./VersionsAdminTablePaginationActions";
...
const EnhancedTable = ({ columns, data, onRowClick }) => {
  ...
  const paramsDataPageIndex = useSelector(
    ({ versionsAdminApp }) => versionsAdminApp.versions.paramsData.PageIndex
  );
  const paramsDataPageSize = useSelector(
    ({ versionsAdminApp }) => versionsAdminApp.versions.paramsData.PageSize
  );
  const paramsDataCount = useSelector(
    ({ versionsAdminApp }) => versionsAdminApp.versions.paramsData.Count
  );
  ...
  return (
    <div className="flex flex-col w-full min-h-full sm:border-1 sm:rounded-16 overflow-hidden">
                ...
                >
                  {row.cells.map((cell) => {
                    return (
                      ...
                      >
                        {cell.column.Header === "Archived Date"
      ...
      <TablePagination
        ...
        ActionsComponent={VersionsAdminTablePaginationActions}
      />
    </div>
  );
};
