...
import FilesAdminTablePaginationActions from "./FilesAdminTablePaginationActions";
import { useDispatch, useSelector } from "react-redux";
import { useTranslation } from "react-i18next";

import { changeParamsDataPageIndex, getFiles } from "./store/filesAdminSlice";
...
const EnhancedTable = ({ columns, data, onRowClick }) => {
  ...
  const paramsDataPageIndex = useSelector(
    ({ filesAdminApp }) => filesAdminApp.files.paramsData.PageIndex
  );
  const paramsDataPageSize = useSelector(
    ({ filesAdminApp }) => filesAdminApp.files.paramsData.PageSize
  );
  const paramsDataCount = useSelector(
    ({ filesAdminApp }) => filesAdminApp.files.paramsData.Count
  );
  ...
  return (
    <div className="flex flex-col w-full min-h-full sm:border-1 sm:rounded-16 overflow-hidden">
      ...
      <TablePagination
        ...
        ActionsComponent={FilesAdminTablePaginationActions}
      />
    </div>
  );
};
