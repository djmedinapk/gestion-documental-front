...
import { handleDeleteFileDialog } from "./store/explorerSlice";
...
const DeleteFileDialog = () => {
  ...
  const dialog = useSelector(
    ({ explorerApp }) => explorerApp.explorer.deleteFileDialog
  );
  ...
  const handleDeleteFile = () => {
    dispatch(removeFile(selectedItem.id)).then(({ payload }) => {
      if (payload.status === 200) {
        dispatch(
          showMessage({
